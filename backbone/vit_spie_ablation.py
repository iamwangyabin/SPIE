...
class ZeroLinearAdapter(nn.Module):
    """Adapter branch that intentionally contributes no delta."""
...
class MLPZeroAdapter(nn.Module):
    """No-adapter ablation branch with the same call surface as LoRA/VeRA."""

    def __init__(self, dim, mlp_hidden_dim):
        super().__init__()
        self.fc1_lora = ZeroLinearAdapter(mlp_hidden_dim)
        self.fc2_lora = ZeroLinearAdapter(dim)
...
class MLPMixedAdapterBlock(nn.Module):
    ...
    def forward(
        ...
        if num_backbone_tokens is not None:
            ...
            if x.shape[1] > num_backbone_tokens:
                ...
                if expert_lora is not None:
                    hidden_expert = hidden_expert + self._apply_expert_adapter_layer(
                        expert_lora.fc1_lora,
        ...
        if num_backbone_tokens is not None:
            ...
            if x.shape[1] > num_backbone_tokens:
                ...
                if expert_lora is not None:
                    out_expert = out_expert + self._apply_expert_adapter_layer(
                        expert_lora.fc2_lora,
    ...
    @staticmethod
    def _apply_expert_adapter_layer(adapter_layer, x, vera_projection):
        if isinstance(adapter_layer, VeraLinear):
            return adapter_layer(x, vera_projection)
        return adapter_layer(x)


class VisionTransformer(TunaMaxVisionTransformer):
    """Ablation-only SPiE ViT with switchable expert adapter branches."""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.expert_adapter_type = str(expert_adapter_type).lower()
        if self.expert_adapter_type not in {"vera", "lora", "none"}:
            raise ValueError(
                f"expert_adapter_type must be one of ['vera', 'lora', 'none'], got {expert_adapter_type}"
            )
        self.expert_lora_rank = int(expert_lora_rank)
        self.expert_lora_alpha = float(expert_lora_alpha)
    ...
    def init_adapters(self):
        ...
        for block in self.blocks:
            ...
            if self.expert_adapter_type == "vera":
            ...
            elif self.expert_adapter_type == "lora":
                adapter = MLPLoRAAdapter(
                    dim=dim,
                    mlp_hidden_dim=mlp_hidden_dim,
                    rank=self.expert_lora_rank,
                    alpha=self.expert_lora_alpha,
                )
            else:
                adapter = MLPZeroAdapter(dim=dim, mlp_hidden_dim=mlp_hidden_dim)
    ...
    def _apply_batched_expert_adapter(self, x, expert_loras, shared_proj):
        if not expert_loras:
            return torch.zeros_like(x)

        if not all(isinstance(expert_lora, VeraLinear) for expert_lora in expert_loras):
            return torch.stack(
                [expert_lora(x[expert_idx]) for expert_idx, expert_lora in enumerate(expert_loras)],
                dim=0,
            )
    ...
    def _forward_expert_mlp_parallel(
        ...
        hidden = hidden + self._apply_batched_expert_adapter(
            norm2_x,
        ...
        out = out + self._apply_batched_expert_adapter(
            hidden,
...
def vit_base_patch16_224_spie(pretrained=False, **kwargs):
    del pretrained
...
def vit_base_patch16_224_in21k_spie(pretrained=False, **kwargs):
    del pretrained
    model = VisionTransformer(
        patch_size=16,
        embed_dim=768,
...
def vit_base_patch16_224_spie_ablation(pretrained=False, **kwargs):
    return vit_base_patch16_224_spie(pretrained=pretrained, **kwargs)


def vit_base_patch16_224_in21k_spie_ablation(pretrained=False, **kwargs):
    return vit_base_patch16_224_in21k_spie(pretrained=pretrained, **kwargs)
