...
class MLPLoRAAdapterBlock(nn.Module):
    """TunaMax block wrapper with split shared/expert MLP LoRA branches."""
    ...
    def forward(self, x, shared_lora=None, expert_lora=None, attn_mask=None, num_backbone_tokens=None):
        ...
        if num_backbone_tokens is not None:
            ...
            if x.shape[1] > num_backbone_tokens:
                ...
                if expert_lora is not None:
                    hidden_expert = hidden_expert + expert_lora.fc1_lora(norm2_x[:, num_backbone_tokens:, :])
        ...
        if num_backbone_tokens is not None:
            ...
            if x.shape[1] > num_backbone_tokens:
                ...
                if expert_lora is not None:
                    out_expert = out_expert + expert_lora.fc2_lora(hidden[:, num_backbone_tokens:, :])
...
class VisionTransformer(TunaMaxVisionTransformer):
    """ViT with shared backbone LoRA plus isolated expert-token LoRA branches."""
...
def vit_base_patch16_224_spie_v6(pretrained=False, **kwargs):
    del pretrained
...
def vit_base_patch16_224_in21k_spie_v6(pretrained=False, **kwargs):
    del pretrained
    model = VisionTransformer(
        patch_size=16,
        embed_dim=768,
...
vit_base_patch16_224_spiev6 = vit_base_patch16_224_spie_v6
vit_base_patch16_224_in21k_spiev6 = vit_base_patch16_224_in21k_spie_v6
