...
class MLPLoRAAdapter(nn.Module):
    """Single LoRA branch applied to the MLP output update."""

    def __init__(self, dim, rank=8, alpha=1.0):
        super().__init__()
        self.mlp_lora = LoRALinear(dim, dim, rank=rank, alpha=alpha)

        # Preserve the adapter-like attribute used by existing optional orth code.
        self.up_proj = self.mlp_lora.up_proj

    def forward(self, x):
        return self.mlp_lora(x)


class MLPLoRAAdapterBlock(nn.Module):
    """TunaMax block wrapper with split shared/expert MLP-output LoRA branches."""
    ...
    def forward(self, x, shared_lora=None, expert_lora=None, attn_mask=None, num_backbone_tokens=None):
        x = x + self.drop_path(self._forward_attention(self.norm1(x), attn_mask=attn_mask))

        residual = x
        norm2_x = self.norm2(x)
        hidden = self.mlp_drop(self.act(self.fc1(norm2_x)))
        out = self.fc2(hidden)
        ...
        if num_backbone_tokens is not None:
            ...
            if shared_lora is not None:
                out_backbone = out_backbone + shared_lora(norm2_backbone)

            if x.shape[1] > num_backbone_tokens:
                out_expert = out[:, num_backbone_tokens:, :]
                if expert_lora is not None:
                    out_expert = out_expert + expert_lora(norm2_x[:, num_backbone_tokens:, :])
...
class VisionTransformer(TunaMaxVisionTransformer):
    """ViT with shared backbone MLP-LoRA plus isolated expert-token MLP-LoRA branches."""
...
def vit_base_patch16_224_spie_v10(pretrained=False, **kwargs):
    del pretrained
...
def vit_base_patch16_224_in21k_spie_v10(pretrained=False, **kwargs):
    del pretrained
    model = VisionTransformer(
        patch_size=16,
        embed_dim=768,
...
vit_base_patch16_224_spiev10 = vit_base_patch16_224_spie_v10
vit_base_patch16_224_in21k_spiev10 = vit_base_patch16_224_in21k_spie_v10
