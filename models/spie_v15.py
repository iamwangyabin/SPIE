...
class Learner(BaseLearner):
    """SPiE v15 with shared classification and task-local expert verification."""
    ...
    def __init__(self, args):
        ...
        self._network = SPIEV15Net(args, True)
        ...
        logging.info(
            "SPiE v15 shared branch: task0 epochs=%s lr=%s, incremental epochs=%s lr=%s, freeze_shared_lora_after_task0=%s.",
    ...
    def _select_candidate_tasks(self, topk_indices):
        ...
        for row in topk_indices.tolist():
            row_tasks = []
            row_seen = set()
            for class_idx in row[: self.verifier_topk]:
                if class_idx < 0:
                    continue
    ...
    def _eval_nme(self, loader, class_means):
        ...
        for _, (_, inputs, targets) in enumerate(loader):
            ...
            with torch.no_grad():
                ...
                shared_topk = self._predict_topk(shared_logits)
                task_similarities = self._compute_task_local_similarities(inputs, shared_logits, shared_topk)
                predicts = self._rerank_topk(shared_logits, shared_topk, task_similarities)
