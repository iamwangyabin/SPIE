...
class Learner(BaseLearner):
    ...
    def __init__(self, args):
        ...
        self.spie_disable_experts = bool(args.get("spie_disable_experts", False))
        self.spie_disable_replay = bool(args.get("spie_disable_replay", False))
        self.synthetic_samples_per_class = int(args.get("synthetic_samples_per_class", self.batch_size * 5))
        self.synthetic_samples_per_class = max(self.synthetic_samples_per_class, 1)
        ...
        for name, param in self._network.backbone.named_parameters():
            expert_param = "cur_adapter" in name or "cur_expert_tokens" in name
            shared_param = "cur_shared_adapter" in name
            param.requires_grad = shared_param or (expert_param and not self.spie_disable_experts)
        ...
        if self.spie_disable_experts:
            logging.info("SPiE ablation: per-task expert branch is disabled.")
        if self.spie_disable_replay:
            logging.info("SPiE ablation: low-rank distributional replay is disabled.")
    ...
    def incremental_train(self, data_manager):
        ...
        if not self.spie_disable_experts:
            self._network.append_expert_head(current_task_size)
        logging.info("Learning on {}-{}".format(self._known_classes, self._total_classes))

        if not self.spie_disable_experts and self._should_reset_task_modules():
    ...
    def _set_expert_head_requires_grad(self, task_id, requires_grad):
        if self.spie_disable_experts or task_id >= len(self._network.expert_heads):
            return
    ...
    def _compute_shared_cls_mean(self, model):
        ...
        for class_idx in range(self._known_classes, self._total_classes):
            ...
            elif self.args["ca_storage_efficient_method"] in {"mean", "mean_only"}:
                self.shared_cls_cov[class_idx] = None
            else:
    ...
    def _classifier_align_module(self, classifier, mean_dict, cov_dict, stage, run_epochs, lr):
        ...
        for epoch in prog_bar:
            sampled_data = []
            sampled_label = []
            num_sampled_pcls = self.synthetic_samples_per_class
            ...
            if self.args["ca_storage_efficient_method"] in ["covariance", "variance"] or self._is_diag_lowrank_ca_method():
                ...
                for class_idx in range(self._total_classes):
                    if self.args["decay"]:
                        task_id = self._class_to_task_id(class_idx)
                        decay = (task_id + 1) / (self._cur_task + 1) * 0.1
                        mean = torch.tensor(mean_dict[class_idx], dtype=torch.float32).to(self._device) * (0.9 + decay)
                    else:
                        mean = mean_dict[class_idx].to(self._device)
                    sampled_data.append(mean.unsqueeze(0).repeat(num_sampled_pcls, 1))
                    sampled_label.extend([class_idx] * num_sampled_pcls)
            else:
                raise NotImplementedError
