#!/usr/bin/env python3
"""
Post-hoc fusion sweep for SPIE v18.

What it does
------------
1. Loads a finished SPIE v18 checkpoint.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="SPIE v18 post-hoc fusion sweep")
    ...
    parser.add_argument("--output-dir", type=str, default="fusion_sweep_v18_out", help="Directory to save cache + CSVs.")
...
def rebuild_spie_v18_to_checkpoint(learner, data_manager: DataManager, checkpoint: Dict[str, Any]) -> None:
...
def method_v18_relation_all_classes(
    shared_logits: np.ndarray,
    metric: np.ndarray,
    task_starts: np.ndarray,
    task_ends: np.ndarray,
...
def run_sweep(
    ...
    pred = method_v18_relation_all_classes(
    ...
    push_row("v18_builtin_relation", pred, metric="sim_cos", alpha=1.0)
...
def main() -> None:
    ...
    if norm_args.get("model_name", "spie_v18").lower() != "spie_v18":
        print(f"[warn] model_name in config is {norm_args.get('model_name')!r}; overriding to 'spie_v18'.")
        norm_args["model_name"] = "spie_v18"
    ...
    rebuild_spie_v18_to_checkpoint(learner, data_manager, checkpoint)
